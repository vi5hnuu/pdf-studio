                    ...
                    <React.Fragment key={label}>
                        <div className="flex flex-col items-center gap-2 flex-shrink-0">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-200
                                ${completed ? 'bg-green-500 text-white' : active ? 'bg-blue-600 text-white ring-4 ring-blue-100' : 'bg-slate-100 text-slate-400'}`}>
                                {completed ? (
                        ...
                        {i < steps.length - 1 && (
                            <div className={`h-0.5 flex-1 mx-2 mt-4 transition-colors duration-200 ${i < activeStepIndex ? 'bg-green-400' : 'bg-slate-200'}`} />
                        )}
                    </React.Fragment>
                );
            })}

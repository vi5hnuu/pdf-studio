...
export function DragDrop({ children, onUpdateItemsOrder }: Readonly<{
    onUpdateItemsOrder: (pPos: number, curPos: number) => void;
    children: React.ReactElement[];
}>) {
    function onDrag(e: any) { e.preventDefault(); }
    function onDragEnd(e: any) { e.preventDefault(); }
    ...
    function onDragEnter(e: any) { e.preventDefault(); }
    function onDragLeave(e: any) { e.preventDefault(); }
    function onDragExit(e: any) { e.preventDefault(); }
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 rounded-xl border border-slate-200 bg-slate-50 gap-4 p-6 max-h-[45rem] overflow-auto">
            {children.map((child, childNo) => (
                <div
                    key={child.key}

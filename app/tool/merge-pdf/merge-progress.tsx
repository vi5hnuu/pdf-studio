...
export function MergeProgress({ files }: { files: FileData[] }) {
    ...
    async function startMerge() {
        ...
        await runToolRequest({
            url: ToolsApi.mergePdf,
            formData,
            fallbackFilename: 'merge-pdf.pdf',
    ...
    const statusText =
        step === Step.UPLOAD ? 'Uploading files...' :
        ...
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            {step !== Step.IDLE && (
                <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                        ...
                        {step !== Step.PROCESS && (
                            <span className="tabular-nums text-slate-400 dark:text-slate-500">{Math.round(progress)}%</span>
                        )}
                    </div>
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    ...
                    <p className="text-center text-xs text-slate-400 dark:text-slate-500">Your merged file will download automatically</p>
                </div>
            )}
        </div>
    );

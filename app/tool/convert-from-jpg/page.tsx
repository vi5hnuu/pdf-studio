...
export default function ConvertFromJpg() {
    ...
    async function startConvert() {
        ...
        await runToolRequest({
            url: ToolsApi.convertFromJpg,
            formData,
            fallbackFilename: 'convert-from-jpg.png',
    ...
    return (
        <div className="flex-1 flex flex-col">
            <div className="bg-gradient-to-r from-rose-500 to-pink-600 text-white px-4 md:px-8 py-2.5 flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-2.5">
                    <div className="w-8 h-8 bg-white/20 rounded-sm flex items-center justify-center flex-shrink-0">
                        <img src="/tools/convert-from-jpg.svg" alt="" className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-base font-semibold leading-tight">JPG to PNG / BMP</h1>
                        <p className="text-xs opacity-75 leading-tight">Convert JPEG images to PNG or BMP format</p>
                    </div>
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeStep === 1 && (
                        <div className="max-w-sm mx-auto space-y-6 py-4">
                            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Output format</p>
                            <div className="grid grid-cols-2 gap-4">
                                {(['PNG', 'BMP'] as OutputFormat[]).map(fmt => (
                                    <button
                                        ...
                                        className={`flex flex-col items-center gap-2 p-5 rounded-sm border-2 transition-all ${format === fmt ? 'border-rose-500 bg-rose-50 dark:bg-rose-900/25' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'}`}
                                    >
                            ...
                            <div className="bg-rose-50 border border-rose-100 rounded-sm px-4 py-3 text-xs text-rose-700 dark:bg-rose-900/20 dark:border-rose-900 dark:text-rose-300">
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    <ToolCostBadge toolId="convert-from-jpg" file={fileData?.file} />
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())}
                                            placeholder="image"
                                            className="w-full px-2.5 py-1.5 rounded-sm border border-slate-200 text-sm outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-50 dark:border-slate-700" />
                                    </div>
                                    <button onClick={startConvert}
                                        className="w-full py-2.5 rounded-sm bg-rose-500 text-white font-semibold text-sm hover:bg-rose-600 transition-colors shadow-sm">
                                        Convert & Download .{format.toLowerCase()}
                                    </button>
                                    <p className="text-center text-xs text-slate-400 dark:text-slate-500">Your converted image will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <ToolSeoSection
                        ...
                        features={[
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-rose-600 dark:text-rose-400"><path d="M5 12h14M12 5l7 7-7 7"/></svg>, title: 'JPG → PNG or BMP', description: 'Choose between lossless PNG (with transparency) or uncompressed BMP.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-rose-600 dark:text-rose-400"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>, title: 'No quality loss on decode', description: 'The JPEG is fully decoded before re-encoding — no additional lossy step.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-rose-600 dark:text-rose-400"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"/></svg>, title: 'No data retention', description: 'Files are deleted from our servers after conversion.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-rose-600 dark:text-rose-400"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>, title: 'Free to use', description: 'No subscription or account required.' },
                        ]}
                        faqs={[
            ...
            <div className="sticky bottom-0 z-30 flex-shrink-0 bg-white border-t border-slate-200 px-6 py-4 dark:bg-slate-800 dark:border-slate-700">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <button disabled={activeStep === 2 || !fileData} onClick={() => setActiveStep(a => a + 1)}
                        className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm bg-rose-500 text-white text-sm font-semibold hover:bg-rose-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm">

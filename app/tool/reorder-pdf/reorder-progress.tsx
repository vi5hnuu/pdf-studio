...
export function ReorderProgress({file,order}: { file: FileData,order:number[] }) {
    ...
    async function startMerge() {
        ...
        formData.append("reorder-pdf-info", new Blob([JSON.stringify({out_file_name: fileName,order:order})], {type: 'application/json'}))
        ...
        xhr.onload = async () => {
            if (xhr.status !== 200) {
                ...
                console.error('Failed to reorder file:', xhr.status, xhr.statusText);
                return;
            }
    ...
    return <div className='w-full flex flex-col items-center justify-center md:w-3/4'>
        ...
        {step == MergeStep.NONE && <div className='flex flex-col items-center justify-center gap-6'>
            ...
            <Button disabled={!fileName.length} className='mx-auto !rounded-full !bg-blue-50 hover:!bg-blue-100 !px-6 !py-3'
                    onClick={startMerge}>Get Pdf</Button>
        </div>}
    </div>
}
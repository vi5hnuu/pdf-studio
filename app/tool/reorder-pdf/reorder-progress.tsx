...
export function ReorderProgress({ file, order }: { file: FileData; order: number[] }) {
    ...
    async function startReorder() {
        ...
        await runToolRequest({
            url: ToolsApi.reorderPdf,
            formData,
            fallbackFilename: 'reorder-pdf.pdf',
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    ...
                    <button
                        ...
                        className="w-full py-2.5 rounded-sm bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 active:bg-indigo-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
                    >
                        Save Reordered PDF
                    </button>
                    <p className="text-center text-xs text-slate-400 dark:text-slate-500">Your reordered PDF will download automatically</p>
                </div>
            )}
        </div>
    );

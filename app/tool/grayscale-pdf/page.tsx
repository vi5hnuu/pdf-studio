...
export default function GrayscalePdf() {
    ...
    return (
        <div className="flex-1 flex flex-col">
            <div className="bg-gradient-to-r from-zinc-600 to-zinc-800 text-white px-4 md:px-8 py-2.5 flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-2.5">
                    <div className="w-8 h-8 bg-white/20 rounded flex items-center justify-center flex-shrink-0">
                        <img src="/tools/grayscale-pdf.svg" alt="" className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-base font-semibold leading-tight">Grayscale PDF</h1>
                        <p className="text-xs opacity-75 leading-tight">Convert any color PDF to black and white grayscale</p>
                    </div>
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    <ToolCostBadge toolId="grayscale-pdf" file={fileData?.file} />
                                    {/* Warning */}
                                    <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700/50 px-4 py-3 text-sm text-amber-800 dark:text-amber-300 dark:border-amber-800">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())} placeholder="grayscale" className="w-full px-2.5 py-1.5 rounded border border-slate-200 text-sm outline-none focus:border-zinc-400 focus:ring-2 focus:ring-zinc-50 dark:border-slate-700" />
                                    </div>
                                    <button onClick={startGrayscale} className="w-full py-2.5 rounded bg-zinc-700 text-white font-semibold text-sm hover:bg-zinc-800 transition-colors shadow-sm">
            ...
            <div className="sticky bottom-0 z-30 flex-shrink-0 bg-white border-t border-slate-200 px-6 py-4 dark:bg-slate-800 dark:border-slate-700">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <button disabled={activeStep === 1 || !fileData} onClick={() => setActiveStep(a => a + 1)} className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded bg-zinc-700 text-white text-sm font-semibold hover:bg-zinc-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm">
                        Next

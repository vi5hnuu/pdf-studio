...
function LayoutPreview({ cols, rows }: { cols: number; rows: number }) {
    ...
    return (
        <div
            className="border-2 border-slate-300 rounded-sm bg-white p-1.5 flex flex-col gap-1 dark:bg-slate-800 dark:border-slate-600"
            style={{ aspectRatio: cols > rows ? '1.41 / 1' : '1 / 1.41', width: 64 }}
        >
            {Array.from({ length: rows }).map((_, r) => (
                <div key={r} className="flex gap-1 flex-1">
                    {Array.from({ length: cols }).map((_, c) => (
                        <div key={c} className="flex-1 rounded-sm bg-slate-100 border border-slate-200 dark:bg-slate-700 dark:border-slate-700" />
                    ))}
                </div>
            ))}
        </div>
            ...
            <div className="bg-gradient-to-r from-violet-600 to-purple-700 text-white px-4 md:px-8 py-2.5 flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-2.5">
                    <div className="w-8 h-8 bg-white/20 rounded-sm flex items-center justify-center flex-shrink-0">
                        <img src="/tools/n-up-pdf.svg" alt="" className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-base font-semibold leading-tight">N-Up Layout</h1>
                        <p className="text-xs opacity-75 leading-tight">Arrange multiple PDF pages onto a single sheet</p>
                    </div>
                        ...
                        <div className="max-w-xl mx-auto space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                {LAYOUTS.map(({ nUp: n, label, description, cols, rows }) => (
                                    <label
                                        key={n}
                                        className={`flex items-start gap-4 p-5 rounded-sm border-2 cursor-pointer transition-all ${nUp === n ? 'border-violet-600 bg-violet-50 dark:bg-violet-900/25 shadow-md' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 bg-white dark:bg-slate-800'}`}
                                    >
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="bg-violet-50 rounded-sm border border-violet-200 px-4 py-3 text-sm text-violet-800 flex items-start gap-3 dark:bg-violet-900/20 dark:border-violet-800 dark:text-violet-300">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input
                                            ...
                                            placeholder={`${nUp}up`}
                                            className="w-full px-2.5 py-1.5 rounded-sm border border-slate-200 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 dark:border-slate-700"
                                        />
                                    </div>
                                    <button
                                        onClick={startConvert}
                                        className="w-full py-2.5 rounded-sm bg-violet-600 text-white font-semibold text-sm hover:bg-violet-700 transition-colors shadow-sm"
                                    >
                                        Convert & Download
                                    </button>
            ...
            <div className="sticky bottom-0 z-30 flex-shrink-0 bg-white border-t border-slate-200 px-6 py-4 dark:bg-slate-800 dark:border-slate-700">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <button
                        ...
                        onClick={() => setActiveStep(a => a + 1)}
                        className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-sm bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
                    >

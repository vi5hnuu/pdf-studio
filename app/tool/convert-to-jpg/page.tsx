...
export default function ConvertToJpg() {
    ...
    async function startConvert() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.convertToJpg, true);
        ...
        xhr.onload = () => {
            ...
            const filename = disp.split('filename=')[1] ?? (outFileName || 'image') + '.jpg';
                        ...
                        <div className="max-w-md mx-auto space-y-6 py-4">
                            <div className="flex flex-col gap-3">
                                <div className="flex justify-between items-baseline">
                                    <label className="text-sm font-semibold text-slate-700">Output JPEG Quality</label>
                                    <span className="text-amber-600 font-bold text-lg">{quality}</span>
                                </div>
                                <input type="range" min={10} max={100} step={5} value={quality} onChange={(e: ChangeEvent<HTMLInputElement>) => setQuality(Number(e.target.value))} className="w-full accent-amber-500" />
                                <div className="flex justify-between text-xs text-slate-400"><span>Smallest</span><span>Best quality</span></div>
                            </div>
                            <p className="text-xs text-slate-400">Quality 90 is the default — recommended for high-fidelity output.</p>
                        </div>
                    )}
                    {activeStep === 2 && (
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            {step !== Step.IDLE && (
                                <div className="space-y-2">
                                    ...
                                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">{step === Step.PROCESS ? <div className="h-full w-full bg-amber-500 animate-pulse" /> : <div className="h-full bg-amber-500 rounded-full transition-all" style={{ width: `${progress}%` }} />}</div>
                                </div>
                            )}
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    <div className="bg-amber-50 rounded-xl border border-amber-200 px-4 py-3 text-sm text-amber-800">Converting to <strong>JPEG at quality {quality}</strong>. Transparent areas will be rendered as white.</div>
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-sm font-medium text-slate-700">Output file name</label>
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())} placeholder="image" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" />
                                    </div>
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your JPEG image will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

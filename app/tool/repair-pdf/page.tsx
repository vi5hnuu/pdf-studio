...
export default function RepairPdf() {
    ...
    async function startRepair() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.repairPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Repair failed. The file may be too severely corrupted.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'repaired') + '.pdf';
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your repaired PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

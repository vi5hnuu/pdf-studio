...
export function PagenoProgress({ file, options }: { file: FileData; options: PageNumbersOptions }) {
    const [step, setStep] = useState<Step>(Step.IDLE);
    const [progress, setProgress] = useState(0);
    const [req, setReq] = useState<XMLHttpRequest | null>(null);
    const [error, setError] = useState<string | null>(null);

    async function startWritePageNumbers() {
        req?.abort();
        const formData = new FormData();
        formData.append('page-numbers-info', new Blob([JSON.stringify(options)], { type: 'application/json' }));
        formData.append('file', file.file);

        const xhr = new XMLHttpRequest();
        setReq(xhr); setError(null);
        xhr.open('POST', ToolsApi.pageNumbers, true);
        ...
        xhr.onload = async () => {
            if (xhr.status !== 200) { setError('Failed to add page numbers'); setStep(Step.IDLE); return; }
            const disposition = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disposition.split('filename=', 2)[1] ?? 'page_numbers.pdf';
            const url = URL.createObjectURL(xhr.response);
            const a = document.createElement('a');
            a.href = url; a.download = filename; a.click();
            URL.revokeObjectURL(url);
        };
        xhr.onerror = () => { setError('Failed to add page numbers'); setStep(Step.IDLE); };
        ...
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    ...
                    <p className="text-center text-xs text-slate-400">Your numbered PDF will download automatically</p>
                </div>
            )}
        </div>
    );

...
export function PagenoProgress({ file, options }: { file: FileData; options: PageNumbersOptions }) {
    ...
    async function startWritePageNumbers() {
        ...
        await runToolRequest({
            url: ToolsApi.pageNumbers,
            formData,
            fallbackFilename: 'page-numbers.pdf',
        ...
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    <button onClick={startWritePageNumbers} className="w-full py-2.5 rounded-sm bg-green-600 text-white font-semibold text-sm hover:bg-green-700 active:bg-green-800 transition-colors shadow-sm">
                        Add Page Numbers
                    </button>

...
export default function FlattenPdf() {
    ...
    async function startFlatten() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.flattenPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Flatten failed. Please try again.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'flattened') + '.pdf';
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-stone-500 focus:ring-2 focus:ring-stone-100"
                                        />
                                    </div>
                                    <button
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your flattened PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

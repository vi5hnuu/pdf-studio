...
export default function WatermarkPdf() {
    ...
    async function startWatermark() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.watermarkPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Watermark failed'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'watermarked') + '.pdf';
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeStep === 1 && (
                        <div className="max-w-2xl mx-auto space-y-6">
                            {/* Preview */}
                            <div className="relative h-36 bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden flex items-center justify-center">
                                <div
                                ...
                                <span className="absolute bottom-2 right-3 text-xs text-slate-400">Preview</span>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-50"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">Font size <span className="text-slate-400">({config.fontSize}px)</span></label>
                                ...
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">Opacity <span className="text-slate-400">({Math.round(config.opacity * 100)}%)</span></label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">Angle <span className="text-slate-400">({config.angle}°)</span></label>
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <input type="color" value={config.colorHex} onChange={(e: ChangeEvent<HTMLInputElement>) => upd('colorHex', e.target.value)} className="w-10 h-10 rounded-xl border border-slate-200 cursor-pointer p-0.5" />
                                        <span className="text-sm text-slate-500 font-mono">{config.colorHex}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())} placeholder="watermarked" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-50" />
                                    </div>
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your watermarked PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

...
export default function StampPdf() {
    ...
    function handleStamp(e: ChangeEvent<HTMLInputElement>) {
        const f = (Object.values(e.target.files ?? {}) as File[])[0];
        if (!f) return;
    ...
    async function startStamp() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.stampPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Stamp failed. Please check both PDF files and try again.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'stamped') + '.pdf';
                    ...
                    {activeStep === 0 && (
                        <div className="space-y-6 max-w-2xl mx-auto">
                            <div className="space-y-2">
                                <p className="text-sm font-medium text-slate-700">Source PDF <span className="text-slate-400 font-normal">(the file to stamp)</span></p>
                                <ChooseFiles id="source-file-upload" single accept={['application/pdf']} onChange={handleSource} />
                                {sourceFile && <p className="text-sm text-center text-slate-500">Selected: <strong>{sourceFile.file.name}</strong></p>}
                            </div>
                            <div className="space-y-2">
                                <p className="text-sm font-medium text-slate-700">Stamp PDF <span className="text-slate-400 font-normal">(first page is used as the stamp)</span></p>
                                <ChooseFiles id="stamp-file-upload" single accept={['application/pdf']} onChange={handleStamp} />
                                {stampFile && <p className="text-sm text-center text-slate-500">Selected: <strong>{stampFile.file.name}</strong></p>}
                            </div>
                        </div>
                    )}
                    ...
                    {activeStep === 1 && (
                        <div className="max-w-md mx-auto space-y-5">
                            <div className="flex flex-col gap-1.5">
                                ...
                                />
                                <div className="flex justify-between text-xs text-slate-400"><span>5%</span><span>100%</span></div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">From page <span className="text-slate-400 font-normal">(0-indexed, optional)</span></label>
                                    <input
                                        type="number"
                                        min={0}
                                        value={fromPage}
                                        ...
                                        placeholder="0"
                                        className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-100"
                                    />
                                </div>
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">To page <span className="text-slate-400 font-normal">(0-indexed, optional)</span></label>
                                    <input
                                        type="number"
                                        min={0}
                                        value={toPage}
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your stamped PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

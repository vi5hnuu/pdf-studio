...
export default function CompressPdf() {
    ...
    async function startCompress() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.compressPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Compression failed'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'compressed') + '.pdf';
            ...
            <div className="bg-gradient-to-r from-slate-700 to-slate-900 text-white px-6 md:px-10 py-5 flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-4">
                    <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center flex-shrink-0">
                        <img src="/tools/compress-pdf.svg" alt="" className="w-7 h-7" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-xl font-bold">Compress PDF</h1>
                        <p className="text-sm opacity-75 mt-0.5">Reduce PDF file size with smart image compression</p>
                    </div>
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeStep === 1 && (
                        <div className="max-w-2xl mx-auto space-y-4">
                            <p className="text-sm text-slate-500 text-center">Choose how aggressively to compress your PDF.</p>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {LEVELS.map(({ value, label, quality, hint, recommended }) => (
                                    <label
                                        key={value}
                                        className={`relative flex flex-col gap-3 p-5 rounded-2xl border-2 cursor-pointer transition-all ${level === value ? 'border-slate-700 bg-slate-50 shadow-md' : 'border-slate-200 hover:border-slate-300 bg-white'}`}
                                    >
                                        ...
                                        <div className="text-xs space-y-1">
                                            <div className="flex justify-between"><span className="text-slate-400">JPEG quality</span><span className="font-medium text-slate-700">{quality}</span></div>
                                        </div>
                                    </label>
                                ))}
                            </div>
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-100"
                                        />
                                    </div>
                                    <button
                                        onClick={startCompress}
                                        className="w-full py-3.5 rounded-xl bg-slate-700 text-white font-semibold text-sm hover:bg-slate-800 transition-colors shadow-sm"
                                    >
                                        Compress & Download
                                    </button>
                                    <p className="text-center text-xs text-slate-400">Your compressed PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <ToolSeoSection
                        ...
                        features={[
                            ...
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-slate-600"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>, title: 'Fast processing', description: 'Each page is rendered and re-encoded server-side in seconds.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-slate-600"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"/></svg>, title: 'No data retention', description: 'Files are deleted from our servers immediately after processing.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-slate-600"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>, title: 'Free to use', description: 'No subscription, no account — just upload and compress.' },
                        ]}
                        faqs={[
            ...
            <div className="flex-shrink-0 bg-white border-t border-slate-200 px-6 py-4">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <button
                        ...
                        onClick={() => setActiveStep(a => a + 1)}
                        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-700 text-white text-sm font-semibold hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
                    >

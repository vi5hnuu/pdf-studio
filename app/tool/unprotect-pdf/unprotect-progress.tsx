...
export function UnprotectProgress({ file, options }: { file: FileData; options: UnprotectOptions }) {
    ...
    async function startUnProtect() {
        ...
        xhr.onload = async () => {
            if (xhr.status !== 200) { setError('Failed to unlock file — check your password and try again'); setStep(Step.IDLE); return; }
        ...
        xhr.onerror = () => { setError('Failed to unlock file'); setStep(Step.IDLE); };
        ...
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    ...
                    <p className="text-center text-xs text-slate-400">Your unlocked file will download automatically</p>
                </div>
            )}
        </div>
    );

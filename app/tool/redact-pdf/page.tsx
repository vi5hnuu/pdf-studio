...
export default function RedactPdf() {
    ...
    function startRedact() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.redactPdf, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Redaction failed. Please check your file and try again.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'redacted') + '.pdf';
                        ...
                        <div className="max-w-2xl mx-auto space-y-5">
                            ...
                            <div className="space-y-4">
                                {regions.map((region, idx) => (
                                    <div key={region.id} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3">
                                        <div className="flex items-center justify-between">
                                            <span className="text-sm font-semibold text-slate-700">Region {idx + 1}</span>
                                            {regions.length > 1 && (
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs font-medium text-slate-600">Page <span className="text-slate-400">(0-indexed)</span></label>
                                                <input type="number" min={0} value={region.page}
                                                    ...
                                                    className="px-3 py-2 rounded-xl border border-slate-200 text-sm outline-none focus:border-zinc-500 focus:ring-2 focus:ring-zinc-100" />
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs font-medium text-slate-600">X <span className="text-slate-400">(pt)</span></label>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs font-medium text-slate-600">Y <span className="text-slate-400">(pt)</span></label>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs font-medium text-slate-600">Width <span className="text-slate-400">(pt)</span></label>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs font-medium text-slate-600">Height <span className="text-slate-400">(pt)</span></label>
                            ...
                            <button type="button" onClick={addRegion}
                                className="w-full py-2.5 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 text-sm font-medium hover:border-zinc-400 hover:text-zinc-700 transition-colors">
                                + Add another region
                            </button>
                        </div>
                    )}
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())}
                                            ...
                                            className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-zinc-500 focus:ring-2 focus:ring-zinc-100" />
                                    </div>
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your redacted PDF will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}

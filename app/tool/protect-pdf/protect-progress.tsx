...
export function ProtectProgress({ file, options }: { file: FileData; options: ProtectOptions }) {
    ...
    async function startProtect() {
        ...
        formData.append('protect-pdf-info', new Blob([JSON.stringify(options)], { type: 'application/json' }));
        formData.append('file', file.file);

        const xhr = new XMLHttpRequest();
        setReq(xhr); setError(null);
        xhr.open('POST', ToolsApi.protectPdf, true);
        ...
        xhr.onload = async () => {
            if (xhr.status !== 200) { setError('Failed to protect file'); setStep(Step.IDLE); return; }
            const disposition = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disposition.split('filename=', 2)[1] ?? 'protected.pdf';
            const url = URL.createObjectURL(xhr.response);
            const a = document.createElement('a');
            a.href = url; a.download = filename; a.click();
            URL.revokeObjectURL(url);
        };
        xhr.onerror = () => { setError('Failed to protect file'); setStep(Step.IDLE); };
        ...
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    ...
                    <p className="text-center text-xs text-slate-400">Your protected file will download automatically</p>
                </div>
            )}
        </div>
    );

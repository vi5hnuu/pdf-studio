...
export function ProtectProgress({ file, options }: { file: FileData; options: ProtectOptions }) {
    ...
    async function startProtect() {
        ...
        await runToolRequest({
            url: ToolsApi.protectPdf,
            formData,
            fallbackFilename: 'protect-pdf.pdf',
    ...
    return (
        <div className="w-full max-w-md mx-auto flex flex-col gap-6 py-10 px-4">
            <ToolCostBadge toolId="protect-pdf" file={file.file} />
            {step !== Step.IDLE && (
            ...
            {step === Step.IDLE && (
                <div className="flex flex-col gap-4">
                    <button onClick={startProtect} className="w-full py-2.5 rounded-sm bg-red-600 text-white font-semibold text-sm hover:bg-red-700 active:bg-red-800 transition-colors shadow-sm">
                        Protect PDF
                    </button>

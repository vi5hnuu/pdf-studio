        ...
        <div style={{ marginLeft: `${depth * 20}px` }}>
            <div className="flex items-center gap-2 py-1.5 group">
                ...
                <input
                    ...
                    className="flex-1 min-w-0 px-2.5 py-1.5 rounded-lg border border-slate-200 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 bg-white"
                />
                {/* Page */}
                <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <input
                        ...
                        className="w-14 px-2 py-1.5 rounded-lg border border-slate-200 text-sm text-center outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50 bg-white"
                    />
                </div>
                {/* Add child */}
                <button type="button" onClick={() => onAddChild(node._id)}
                    title="Add child bookmark"
                    className="flex-shrink-0 p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors opacity-0 group-hover:opacity-100">
...
export default function EditBookmarks() {
    ...
    async function loadBookmarks() {
        ...
        try {
            ...
            const res = await fetch(ToolsApi.getBookmarks, { method: 'POST', body: formData });
    ...
    function startSave() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.editBookmarks, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Save failed. Please try again.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'bookmarks-edited') + '.pdf';
                    ...
                    {activeStep === 1 && (
                        <div className="max-w-2xl mx-auto space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="text-xs text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full">
                                    Hover a row to see actions
                                </div>
                            </div>

                            {/* Header hint */}
                            <div className="flex items-center gap-2 text-xs text-slate-400 px-1 pb-1 border-b border-slate-100">
                            ...
                            <div className="space-y-0.5 min-h-[60px]">
                                {bookmarks.length === 0 ? (
                                    <p className="text-sm text-slate-400 text-center py-8">No bookmarks. Add one below.</p>
                                ) : (
                                    bookmarks.map(node => (
                            ...
                            <button type="button" onClick={addTopLevel}
                                className="w-full py-2.5 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 text-sm font-medium hover:border-indigo-400 hover:text-indigo-600 transition-colors">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())}
                                            ...
                                            className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-50" />
                                    </div>

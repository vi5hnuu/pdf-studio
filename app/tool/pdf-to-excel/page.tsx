...
export default function PdfToExcel() {
    ...
    async function startConvert() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.pdfToExcel, true);
        ...
        xhr.onload = () => {
            ...
            const filename = disp.split('filename=')[1] ?? (outFileName || 'spreadsheet') + '.xlsx';
            ...
            <div className="flex-1 overflow-auto px-6 md:px-10 py-8">
                <div className="max-w-5xl mx-auto">
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            {step !== Step.IDLE && (
                                <div className="space-y-2">
                                    ...
                                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">{step === Step.PROCESS ? <div className="h-full w-full bg-green-600 animate-pulse" /> : <div className="h-full bg-green-600 rounded-full transition-all" style={{ width: `${progress}%` }} />}</div>
                                </div>
                            )}
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    <div className="bg-green-50 rounded-xl border border-green-200 px-4 py-3 text-sm text-green-800">Text is extracted and written into an <strong>.xlsx</strong> spreadsheet — one worksheet per page. Column detection is best-effort.</div>
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-sm font-medium text-slate-700">Output file name</label>
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())} placeholder="spreadsheet" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100" />
                                    </div>
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your Excel file will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}
                    <ToolSeoSection
                        ...
                        features={[
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-600"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>, title: 'One sheet per page', description: 'Each PDF page is extracted into its own worksheet tab in the workbook.' },
                            ...
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-600"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10"/></svg>, title: 'No data retention', description: 'Files are deleted from our servers after conversion.' },
                            { icon: <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-green-600"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>, title: 'Free to use', description: 'No subscription or account required.' },
                        ]}
                        faqs={[

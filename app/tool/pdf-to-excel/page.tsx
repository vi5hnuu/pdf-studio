...
export default function PdfToExcel() {
    ...
    return (
        <div className="flex-1 flex flex-col">
            <div className="bg-gradient-to-r from-green-600 to-emerald-700 text-white px-4 md:px-8 py-2.5 flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-2.5">
                    <div className="w-8 h-8 bg-white/20 rounded-sm flex items-center justify-center flex-shrink-0"><img src="/tools/pdf-to-excel.svg" alt="" className="w-5 h-5" /></div>
                    <div className="flex-1 min-w-0"><h1 className="text-base font-semibold leading-tight">PDF to Excel</h1><p className="text-xs opacity-75 leading-tight">Export PDF text content to a structured Excel spreadsheet</p></div>
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="bg-green-50 rounded-sm border border-green-200 px-4 py-3 text-sm text-green-800 dark:bg-green-900/20 dark:border-green-800 dark:text-green-300">Text is extracted and written into an <strong>.xlsx</strong> spreadsheet — one worksheet per page. Column detection is best-effort.</div>
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Output file name</label>
                                        <input type="text" value={outFileName} onChange={(e: ChangeEvent<HTMLInputElement>) => setOutFileName(e.target.value.trim())} placeholder="spreadsheet" className="w-full px-2.5 py-1.5 rounded-sm border border-slate-200 text-sm outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 dark:border-slate-700" />
                                    </div>
                                    <button onClick={startConvert} className="w-full py-2.5 rounded-sm bg-green-600 text-white font-semibold text-sm hover:bg-green-700 transition-colors shadow-sm">Convert & Download .xlsx</button>

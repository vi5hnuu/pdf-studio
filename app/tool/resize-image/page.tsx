...
export default function ResizeImage() {
    ...
    function startResize() {
        ...
        setError(null);
        xhr.open('POST', ToolsApi.resizeImage, true);
        ...
        xhr.onload = () => {
            if (xhr.status !== 200) { setError('Resize failed. Please check your inputs and try again.'); setStep(Step.IDLE); return; }
            const disp = xhr.getResponseHeader('Content-Disposition') ?? '';
            const filename = disp.split('filename=')[1] ?? (outFileName || 'resized') + '.jpg';
                        ...
                        <div className="max-w-sm mx-auto space-y-6 py-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">Width <span className="text-slate-400 font-normal">(px)</span></label>
                                    <input
                                        type="number"
                                        min={1}
                                        value={width}
                                ...
                                <div className="flex flex-col gap-1.5">
                                    <label className="text-sm font-medium text-slate-700">Height <span className="text-slate-400 font-normal">(px)</span></label>
                                    <input
                                        type="number"
                                        min={1}
                                        value={height}
                            ...
                            >
                                ...
                                <div className="text-left">
                                    ...
                                    <p className="text-xs text-slate-400">If enabled, set only one dimension — the other is calculated proportionally</p>
                                </div>
                            </button>
                        ...
                        <div className="max-w-md mx-auto flex flex-col gap-6 py-8">
                            ...
                            {step === Step.IDLE && (
                                <div className="flex flex-col gap-4">
                                    ...
                                    <p className="text-center text-xs text-slate-400">Your resized image will download automatically</p>
                                </div>
                            )}
                        </div>
                    )}
